...
class TestNewsSelenium(StaticLiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.selenium.find_element(value="id_password").send_keys("lfnf231293")
        button_enter.click()
        # Wait for footer
    ...
    def test_pick_color(self):
        ...
        try:
            self.assertEqual(navbar_el.value_of_css_property("background-color"),
                             "rgba(255, 255, 255, 1)",)
...
class TestNewsPage(TestCase):
    fixtures = ("authapp/fixtures/001_admin_user.json",
                "mainapp/fixtures/001_news.json")
    ...
    def setUp(self):
        ...
        self.client_with_auth.post(
            path_auth, data={"username": "admin", "password": "lfnf231293"})
...
class TestCoursesWithMock(TestCase):
    fixtures = (
        "authapp/fixtures/001_admin_user.json",
        "mainapp/fixtures/006_courses.json",
        "mainapp/fixtures/002_lessons.json",
        "mainapp/fixtures/003_teachers.json",
    )

    def test_page_open_detail(self):
        course_obj = mainapp_models.Courses.objects.get(pk=2)
        path = reverse("mainapp:courses_detail", args=[course_obj.pk])
        with open("mainapp/fixtures/005_feedback_list_2.bin", "rb") as inpf, mock.patch("django.core.cache.cache.get") as mocked_cache:
